...
| `CRAWL4AI_API_TOKEN` | optional | `src/lib/team-stats.ts:47`, `scripts/gather-team-stats.ts` | Bearer token for crawl4ai. Read by the gather tooling **and** the live on-read team-stats refresh (`/api/team-stats/refresh`). Both send no usable token when unset (the in-network crawl4ai service does not enforce auth). |
