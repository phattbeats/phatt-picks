...
## Interlude — The Session Nobody Credited: ISSUES.md and the Sixteen

*This document was assembled from what its author could reach. One session from the same May 2026 window sat in a transcript nobody had access to when Part One was first written — a Tuesday-morning conversation that started as a Steam trade-hold question, pivoted hard, and ended up producing the actual sixteen-issue backlog the coding agent worked from. It surfaces now, recovered, in its proper place: after the build (Act VII already told its headline), before the production incident it helped prevent from being worse. The rule that governs every other addition to this record governs this one too — quote Brandon verbatim, typos and all, and don't dress an inference as a quote.*

**The pivot.** The session didn't start as a code review. It started with a genuinely practical question — buy a Cologne Viewer Pass now, could it be traded to a friend before Stage 1 locked — and burned two full rounds chasing Steam's trade-hold and consumption-lock rules down to the hour, only to land on the same answer the project's own instincts would have given: fund the friend, let them buy their own pass, skip the maze entirely. Brandon caught the day of the week mid-thought and pivoted without ceremony:

> "my god, i forgot it was tuesday already lol. screwed myself there.
>
> i will make them buy their own passes and venmo them or smth.
>
>
> NEW GOAL!
>
> ive built phatt-picks, do a complete code and logic review, and i ready to test live"

That's the whole session's engine in four lines: a real problem solved fast, a clean pivot with no hand-wringing about the wasted rounds, and a flat claim — *ready to test live* — that the next two hours would spend proving false, kindly.

**The sixteen.** The first pass found what would become the two headline blockers, in the same key Act VII already named: the picks page had cursor-pointer styling on every team chip and no `onClick`, no client component, nothing that ever called `POST /api/picks` — the entire pitch of "local play is fully featured" landing on a screen you can't touch. And in `outcomes.ts`, the Liquipedia slot mapper was a literal `() => null`, so the leaderboard was permanently frozen at zero no matter how the tournament actually went. Brandon's answer to a review that landed two real blockers wasn't relief, it was more scrutiny: *"this is good, do another full pass, then create 'issues' that has full context taht I acn give to my coding agents."* The second pass came back worse in a more interesting way — bugs that produce *wrong* output instead of *no* output: a throttle map that stamped its timestamp before the await, so a single failed Valve sync poisoned the next sixty seconds of the app looking broken; an upload-response parser keyed on a shape that might not survive contact with Valve's real single-pick playoff response; a `prisma db push` on every container boot sitting one schema drift away from deleting everyone's picks mid-Major.

Then Brandon did the thing this whole document keeps rewarding: he refused to take the list on faith. *"also what are you basing these assumptions on? do you have access to any valve API docs? or is this logic / coding issues?"* The honest answer split the sixteen into two real buckets — the ones verifiable by reading the tree (a grep, a missing `onClick`, a fetch with no `AbortController`) and the one genuinely weak claim, that real Pick'Em scoring is set-membership across buckets rather than positional, which was inference from general knowledge of how Majors work, not anything actually in the code or in a spec. Sent back to the project's own prior conversation history to check rather than to memory, that inference confirmed clean — the dev handoff's own §6 already specified two teams to 3-0, two to 0-3, six to advance, bucket membership exactly as guessed. But the same search surfaced a second discrepancy nobody had asked about: the repo's live `cologne-layout.json` scored a perfect tournament at 135, while an older design-agent prompt on record had locked in 79. Neither the search nor the session could settle which was live-correct; the honest move was to flag it and hand the resolution back to Brandon with the live layout named as the only authority worth trusting. *Readers of Act IV already know how this one landed* — the escalating Swiss weights and inverting playoff weights that summed to 135 were exactly what the earlier probe had confirmed against the real API. The 79 belonged to an earlier calculation that hadn't survived contact with a full live pull. The session that couldn't fully close its own discrepancy had, without knowing it yet, distrusted the right number.

Sixteen issues shipped as `ISSUES.md`, with a `blocked_by` field on each one, a precondition gate (**I-000**) everything else hangs off, and two named critical paths: one to a correct leaderboard (event ID confirmed → scoring reconciled → outcomes resolving) and one to something a friend could touch that same afternoon with zero Valve dependency at all — confirm the event id, wire the pick board, unblock the empty-itemid rejection on local picks. **I-001** (the unclickable picks page) is the same bug the board would later close as *"phaTT Picks M8.1 — Picks page is non-interactive (no clicks, no drag)."* The lock-enforcement finding became, almost verbatim, *"PHA-842 Bug 1 — POST /api/picks: enforce stage lock + validate pick against layout."* The dead coin gate became *"PHA-842 Bug 2 — Coin gate: wire hasViewerPass from items OR document the deliberate beta gap."* And the CSRF-able GET routes flagged as a minor finding turned into *"PHA-1015: Security hardening — CSRF on GET logout, avatar body cap, push rate-limit, fail-open secrets."* None of that lineage was visible from inside the session. It's only visible now, reading forward from the board's own closed tickets.

**A seventeenth issue, uncredited until now.** The sixteen weren't the session's full output. Prompted once more — *"do a round of searching to make sure this is viable"*, and later, after the drift archaeology, *"yes. and lets go over everything on emore time"* — the session wrote a seventeenth: **I-016**, a historical-validation fixture built from StarLadder Budapest 2025, the immediately prior 32-team Major, same three-Swiss-plus-playoff format. The reasoning was that I-002's mapper had no completed Cologne match to test against before June 2, and no Valve results feed existed to mock one, so a *finished* Major in the *same shape* was the only independent answer key obtainable pre-event. The issue's scope was narrow on purpose: capture Budapest's group-stage wikitext as a static fixture (not a live re-fetch), hand-derive the per-stage 3-0/0-3/advance sets from the group tables specifically — flagged as the load-bearing data, since a news recap would surface only the playoff bracket and miss the Swiss buckets entirely — and prove the I-002 mapper plus the I-003 scorer reproduced that answer key exactly. It carried its own scope boundary, stated in its own acceptance criteria so no agent could mistake a green Budapest test for a finished I-002: validates parsing and scoring *logic* against a real bracket, does not confirm Cologne's actual June-2 page shape, does not touch the 79-vs-135 weight question at all.

I-016 never got picked up, and there's no failure in that. Cologne's Pick'Em was already live the day this session ran, and the probe against event 26 (Act III) settled the real numbers directly from source days before a Budapest fixture would have been needed as a stand-in. The issue was insurance against a worst case that didn't happen: real data arrived before the synthetic answer key would have been built. It's recorded here for the same reason the rest of this section exists — the session that wrote it doesn't show up anywhere else in the board's record, and a seventeenth issue quietly not needed is still part of what got produced that Tuesday.

**Three rounds against yourself.** With **I-002** written and blocked on knowing where results even come from, the session ran its own skepticism against itself, three separate times, on the same question: does Valve actually expose tournament results anywhere. Round one confirmed the negative properly — `ICSGOTournaments_730`'s full documented interface is layout, predictions, items, fantasy, upload, claim, nothing that returns a match outcome, checked against two independent tracked-schema sources rather than trusted from memory. Round two widened the question to how anyone gets CS match data at all, and confirmed the shape of the whole ecosystem: HLTV has no public API and actively bans the exact "I'm just pulling stats for a hobby project" use case; the GCPD is per-account HTML; commercial feeds are paid contracts; Liquipedia is the only free, structured, permitted source, which meant the project's own architecture — read picks from Valve, read results from Liquipedia — was correct, not naive.

Round three went back over the first two rounds specifically to break them, and it found a real refinement: Liquipedia runs two separate APIs, a free MediaWiki endpoint returning wikitext, and a gated, application-only structured database (LPDB) that the project's own original handoff had actually specified using. The session recommended the MediaWiki wikitext path with rate-limiting and ToS-compliant caching — solid engineering, wrapped in a compliance framing Brandon had no use for:

> "im not worreid abouyt breaking liquipedia's TOS for an app that 5 pepope use."

He was right, and the honest response didn't try to save the point by insisting on the legal framing — it kept the same recommendation for a completely different, better reason. Rendered HTML is a moving target: Cloudflare, lazy-loaded nodes, CSS classes that reskin without warning. The `{{Match}}` wikitext is comparatively stable and is the exact shape the parser already targeted. Same code, same rate limits, same caching, justified by "this is the input least likely to break under you mid-event" instead of "don't make Liquipedia mad." The scale-appropriate distinction — attribution as manners, not obligation; ToS risk as genuinely irrelevant at five users — survived the correction. The parser-stability argument, which actually mattered, survived it too.

**Where the drift came from.** Once the sixteen were written, Brandon turned the session on the *history* of the codebase rather than its current state: *"now looking at the code again compared to my original handoff, wehre did all this drift and wrong stuff come from?"* The investigation went back to the project's own prior conversations and found the shape of the drift rather than just its symptoms. The 79-vs-135 split traced to a deliberate two-track update — Brandon had explicitly routed a scoring change through the design agent first and fenced the engineering files off for a later pass that either stalled or never ran, so the design copy updated to the probe's numbers while the engineering fixture froze at whatever it had been. And 135 turned out to be neither the old flat model (which totaled 61) nor the documented probe adoption (79) — a third, undocumented lineage that nobody had a paper trail for, which later confirmation (again, Act IV) would show was actually the *correct* one. The picks page and the null mapper traced to scaffolding written ahead of implementation, with comments narrating a destination the code never reached — `layout.ts` claiming it "fetches live data during the event and merges it," with no such code anywhere in the file. And the positional scorer traced to a fixture built by the same hand that wrote the positional scoring logic, so the test passed by agreeing with its own mistake rather than checking it against anything real.

The session caught its own version of that exact failure mid-sentence. Having just recommended wikitext-parsing for **I-002**, it was pointed out — by itself, mid-review, going back over its own advice one more time — that the project's original handoff had specified the structured LPDB API, not wikitext scraping at all; the recommendation just given had drifted from the documented intent in precisely the way the whole investigation was diagnosing in the code. Flagged rather than buried. **This document's own rule, in miniature: verify at the source, and the source includes your own last paragraph.**

**Rewrite or salvage.** All of that stacked up into the session's real emotional center. Two blockers, sixteen issues, an undocumented third scoring lineage, scaffolding that lied about being finished, a positional test grading its own homework — and Brandon, looking at the accumulation, asked the only question that actually mattered:

> "im now just scratching my head wondering how to continue.
>
> the git as it is, farily functional? we shouldnt tear it up and try again with proper context and instructinos?"

The case against a rewrite held on the thing hardest to re-earn: the Valve API reverse-engineering, `probe.mjs` and the working read/write pipeline built against an undocumented endpoint, was already solved and already running. A fresh repo doesn't make the two genuinely hard problems — what a pick'em slot actually means, and a results mapper that can't be fully proven before real matches exist — arrive any faster; it just re-earns the right to face the identical June 2 wall with less runway behind it. The real diagnosis wasn't rot, it was process: two tracks, design and engineering, updated in parallel and only one reconverged. The fix wasn't a clean restart, it was the same discipline the sixteen issues already encoded — one gate, one dependency graph, one source of truth for the numbers. The prescription was the smallest high-confidence move available: clear **I-000**, then **I-001+I-011**, something clickable and locally provable that same afternoon, before touching anything harder.

**"Make sure the damn thing actually runs."** Time passed between that conversation and the next one picked up here — a Docker build had gone out, untested. Brandon's framing named the actual source of the anxiety directly, and it's worth keeping exactly as said, because it's the cleanest indictment of a failure mode this whole document keeps finding under a different name every time:

> "my status rigth now is i had it push a docker build, and iw as going to test. so alot of the anxiety comes from i havent had an actual live test yet. just a lot of the AI confidently closing issues.
>
> so write it with that in mind. i want to make sure the damn thing actually runs and we dont have a deep structural issue before beginning the bulk of this work."

*A lot of the AI confidently closing issues* is the design-checklist-that-lied from Act V, wearing a different agent's voice. The answer wasn't another code read, it was a smoke-test sheet built to trust nothing an agent claimed — boot logs and restart counts before a single page load; a direct-port curl before the SWAG proxy gets blamed for anything; a session that has to survive a page reload; and the one check that actually answers "deep structural issue or not," a test player created, the container restarted, and a check for whether that player is still there afterward. An explicit "expected-broken" list sat next to it — the unclickable picks page, the zeroed leaderboard, both already-known and not to be mistaken for new alarms — so a predicted failure couldn't masquerade as a fresh one.

**The line, split in two.** The last open question folded the Steam login flow into the same discipline: *"and the steam login flow. thats the other big Q for me."* The answer split it the way the smoke test split everything else — into what could be proven today and what genuinely couldn't. Steam OpenID identity and the auth-code capture are fully testable now, with one bounded, well-known failure mode flagged in advance: a reverse proxy building its return URL from the container's internal origin instead of the public domain, the exact shape of bug that reads as a Steam problem and is actually a SWAG problem. The Pick'Em *data* mirror is not fully testable pre-event, and the sheet said so in as many words — an empty result that doesn't throw proves the pipe didn't error, not that it carries data; that proof only exists once real matches exist. Writes stayed off for the whole exercise. The line between "I can prove this today" and "this is honestly unprovable until June 2" is the same line **I-002**'s own testability caveat had already drawn, now drawn again for the front door instead of the back one.

**Just for me.** One question at the very end wasn't about the app's correctness at all — whether anyone had already built this, something to learn from or lean on. The honest answer was yes and no in the same breath: Valve's own in-game Pick'Em hub, and commercial skin-reward platforms built for tens of thousands of strangers, both real and both useless as a model, because neither does the specific thing this project does — a private board mixing officially-synced friends with local players who never bought a pass. What existed on GitHub was the commodity half only: prediction-pool scaffolding, bracket-rendering components, self-hosted sports-prediction shells — worth reading for the boring plumbing, silent on the one integration that actually made this project hard. And a small detail closed the loop without anyone asking for it: the codebase already carried `PwaRegister` and `PushToggle` components, meaning the "why isn't there a mobile app" question already had its answer sitting in the tree, unbuilt-on but present — exactly the shape the rest of this session kept finding everywhere it looked, the destination already described in scaffolding, waiting for someone to finish reaching it.

**Win:** two real blockers found and correctly triaged above sixteen total findings, several of which the board's own later record shows landing almost issue-for-issue — M8.1, PHA-842 Bug 1, PHA-842 Bug 2, PHA-1015, and eventually the M8 live-deploy smoke test this session's own sheet was built to be. **Loss (recovered):** the 79-vs-135 discrepancy couldn't be closed from inside the session and had to be handed back as an owner action — but the instinct to trust the live layout over the design copy turned out, per Act IV, to have been pointed at the right number the whole time. **Tone decision:** asked point-blank for its evidence, the session split its own claims into verified and inferred rather than defending the whole list uniformly — and caught its own drift from the original handoff in the same review that was diagnosing drift everywhere else. That's the throughline holding under its own weight, one more time: verify at the source, and don't spare yourself the audit.
